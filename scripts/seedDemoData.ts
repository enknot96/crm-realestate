// デモ用の架空データを投入する本体
// seed.ts(手動実行)とreset-demo.ts(日次リセット)の両方から呼ばれる
// 何度実行しても同じ状態になるよう、投入前にデモ対象のテーブルを全部空にしてから作り直す
...
// env.ts経由(getDb())だと起動時に全環境変数を検証してしまい、
// DATABASE_URLしか無いCI環境(reset-demo.yml)で動かせないため、ここだけ直接接続する
...
export async function seedDemoData(): Promise<void> {
  ...
  await db.insert(patrolReports).values([
    ...
    },
    {
      ...
      checklistResults: buildChecklistResults(),
      status: "sent",
      body: wrapReportWithGreeting("本日巡回いたしましたが、各項目とも問題ございませんでした。"),
      generatedBy: "human",
  ...
  await db
    .insert(messageLogs)
    .values(Array.from({ length: 15 }, (_, i) => ({ sentAt: daysAgo(i % 5) })));
